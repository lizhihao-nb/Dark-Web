...
class TaxonomyBuilder:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "sample_collection_name": "content20000",  # 用于构建分类法的样本集合
            "target_collection_name": "content",       # 需要分类的目标集合
            ...
            "min_samples_for_centroid": 1,
        }
        
        # 初始化客户端
        self.client = None
        self.db = None
        self.sample_collection = None  # 样本集合
        self.target_collection = None  # 目标集合
    ...
    def _init_clients(self):
        ...
        try:
            ...
            # 初始化两个集合
            self.sample_collection = self.db[self.config["sample_collection_name"]]
            self.target_collection = self.db[self.config["target_collection_name"]]
            print("MongoDB连接成功")
            print(f"样本集合: {self.config['sample_collection_name']}")
            print(f"目标集合: {self.config['target_collection_name']}")
    ...
    def stage1_summarization(self, sample_size: int = None) -> List[Dict]:
        """
        第一阶段：从样本集合(content20000)直接读取已有的 summary 和 vector
        """
        try:
            if sample_size is None:
                ...
                total_count = self.sample_collection.count_documents({
                    "summary": {"$exists": True, "$ne": "", "$ne": "内容过短，无法生成摘要"}
                })
                ...
                total_count = min(sample_size, self.sample_collection.count_documents({
                    "summary": {"$exists": True, "$ne": "", "$ne": "内容过短，无法生成摘要"}
                }))
    ...
    def classify_all_documents(self, taxonomy: List[Dict]):
        print("=== 开始对目标集合(content)进行一级分类 ===")
        
        self.calculate_category_centroids()
        
        # 修改查询条件：只查找有vector的文档，不需要summary
        try:
            cursor = self.target_collection.find(
                {"vector": {"$exists": True, "$ne": None}},  # 只检查vector
                {"_id": 1, "vector": 1}  # 只需要_id和vector
            )
            total = self.target_collection.count_documents({
                "vector": {"$exists": True, "$ne": None}
            })
            print(f"在目标集合中共找到 {total} 个含有效 vector 的文档，准备分类...")
        except Exception as e:
            print(f"查询目标集合文档失败: {e}")
            return
        ...
        for doc in cursor:
            vector = doc.get("vector")  # 只需要vector
            ...
            if len(batch_for_update) >= 100:
                try:
                    self.target_collection.bulk_write(batch_for_update, ordered=False)
                except Exception as e:
                    print(f"批量更新失败: {e}")
                batch_for_update = []
        ...
        # ========== 导出目标集合的所有文档分类结果 ==========
        all_docs = []
        for doc in self.target_collection.find(
        ...
        self.export_to_json(all_docs, output_path="output/content_taxonomy_results.json")

    def print_categories_simple(self):
        """简单打印所有类别"""
        print("\n" + "="*50)
        print("最终分类法中的所有类别")
        print("="*50)

        if not self.final_taxonomy:
            print("分类法为空，请先运行run_analysis方法")
            return

        for i, item in enumerate(self.final_taxonomy, 1):
            category = item.get("category", "未知")
            print(f"{i:2d}. {category}")

        print(f"\n总计: {len(self.final_taxonomy)} 个类别")

    def run_analysis(self, sample_size_for_taxonomy: int = 500):
        print("=== 第一阶段：基于样本集合(content20000)构建一级分类体系 ===")
        # 从样本集合(content20000)获取数据
...
def main():
    ...
    try:
        # 从content20000抽取5000个样本构建分类法
        # 然后对content集合中的所有文档进行分类
