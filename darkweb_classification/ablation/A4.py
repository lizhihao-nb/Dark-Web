...
class TaxonomyBuilder:
    ...
    def export_to_json(self, documents: List[Dict], output_path: str = "output/full_taxonomy_results1000-4.json"):
        try:
            output_data = []
            for doc in documents:
                summary = doc.get("summary", "").strip()
    ...
    # ==================== 新增：全量导出方法 ====================
    def export_full_results_to_json(self, field_name: str, output_path: str):
        """
        导出所有具有指定分类字段的文档（全量，无采样）
        """
        try:
            cursor = self.collection.find(
                {field_name: {"$exists": True}},
                {"_id": 0, "summary": 1, field_name: 1}
            )
            output_data = []
            for doc in cursor:
                summary = doc.get("summary", "").strip()
                if not summary or summary == "内容过短，无法生成摘要":
                    continue
                category = doc.get(field_name, "未分类").strip()
                output_data.append({
                    "alert": summary,
                    "category": category
                })
            ...
            print(f"✅ 全量结果已导出: {output_path}（共 {len(output_data)} 条）")
        except Exception as e:
            print(f"❌ 导出失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def classify_all_documents(self, taxonomy: List[Dict]):
        ...
        pipeline = [
            ...
            {"$project": {"summary": 1, "category": 1}},
            {"$sample": {"size": 800}}  # 随机取 800 条
        ]
        sample_docs = [
            {"summary": doc["summary"], "category": doc.get("category", "未分类")}
            for doc in self.collection.aggregate(pipeline)
        ]
        self.export_to_json(sample_docs, output_path="output/full_taxonomy_results_sample_800.json")
        # 新增：导出全量结果
        self.export_full_results_to_json(
            field_name="category",
            output_path="output/full_taxonomy_results_static_all.json"
        )

    # ==================== 新增：动态分类方法 ====================
    def classify_all_documents_dynamic(self, taxonomy: List[Dict]):
        """
        对全量文档进行动态扩展分类
        在分类过程中将新文档的向量加入锚点池
        """
        print("=== 开始对全量文档进行动态扩展分类 ===")

        # 初始化动态锚点池：从已计算的质心开始
        self.dynamic_anchors = []
        for category, centroid in self.category_centroids.items():
            self.dynamic_anchors.append((centroid, category))
        print(f"初始锚点池：{len(self.dynamic_anchors)} 个质心")
        ...
        try:
            ...
            print(f"共找到 {total} 个含有效 summary 的文档，准备动态分类...")
        except Exception as e:
            print(f"查询全量文档失败: {e}")
            return
        ...
        for doc in cursor:
            ...
            if vector is None:
                ...
                min_dist = float('inf')
                best_category = "其他"
                for ref_vec, cat in self.dynamic_anchors:
                    try:
                        dist = cosine(vector, ref_vec)
                        if dist < min_dist:
                            min_dist = dist
                            best_category = cat
                    except Exception as e:
                ...
                if min_dist <= self.config["distance_threshold"]:
                    ...
                    # 动态扩展：将新文档向量加入锚点池
                    self.dynamic_anchors.append((vector, category))
                else:
                    category = "其他"
            ...
            batch_for_update.append(
                pymongo.UpdateOne(
                    ...
                    {
                        "$set": {
                            "category_dynamic": category,
                            "classification_dynamic_updated_at": datetime.now()
                        }
                    }
                )
            )
            ...
            processed += 1
            if processed % 100 == 0:
                print(f"动态分类进度: {processed}/{total} (锚点数: {len(self.dynamic_anchors)})")
        ...
        print(f"动态分类完成，共处理 {processed} 个文档，最终锚点数: {len(self.dynamic_anchors)}")
        # 导出全量动态分类结果
        self.export_full_results_to_json(
            field_name="category_dynamic",
            output_path="output/full_taxonomy_results_dynamic_all.json"
        )
    # ====================================================
    ...
    def run_analysis(self, sample_size_for_taxonomy: int = 500):
        ...
        print("\n=== 第四阶段：对全量文档进行动态扩展分类 ===")
        self.classify_all_documents_dynamic(taxonomy)

        print("\n全流程完成！已生成以下文件：")
        print("1. static_sample.json - 800条静态分类采样结果")
        print("2. static_all.json - 全量静态分类结果")
        print("3. dynamic_all.json - 全量动态分类结果")
...
def main():
    ...
    try:
        analyzer.run_analysis(sample_size_for_taxonomy=1000)
