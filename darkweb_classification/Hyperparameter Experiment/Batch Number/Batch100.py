...
class TaxonomyBuilder:
    ...
    def _migrate_category_vectors(self, merged_from: Dict[str, List[str]]):
        ...
        修复：禁止自复制，迁移后删除源类别，避免向量重复叠加。
        """
        if not merged_from:
            return

        # 收集所有被引用的旧类别（用于识别哪些要迁移）
        all_old_cats = set()
        for old_list in merged_from.values():
            all_old_cats.update(old_list)
        ...
        for new_cat, old_list in merged_from.items():
            ...
            for old_cat in old_list:
                if old_cat in self.category_vectors:
                    combined.extend(self.category_vectors[old_cat])
                    # 删除已迁移的旧类别，防止重复
                    del self.category_vectors[old_cat]
            new_vectors[new_cat] = combined

        # 保留未被迁移的类别（如新增类别或未涉及的类别）
        for cat, vecs in self.category_vectors.items():
            if cat not in all_old_cats:
                new_vectors[cat] = vecs

        # 替换为新结构
        self.category_vectors = dict(new_vectors)
                ...
                if dist > self.config["distance_threshold"]:
                    ...
                    if not llm_res.get("fits_existing", True):
                        ...
                        if new_cat:
                            ...
                            self.calculate_category_centroids()
            current_cat_names = {item["category"] for item in taxonomy}
            self.category_vectors = {
                cat: vecs for cat, vecs in self.category_vectors.items()
                if cat in current_cat_names
            }
    ...
    def classify_all_documents(self, taxonomy: List[Dict]):
        ...
        self.export_to_json(all_docs, output_path="output/full_taxonomy_results-100.json")
